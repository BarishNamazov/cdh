...
function showCommandHelp(cmd: string | undefined): void {
  ...
  switch (cmd) {
    ...
    case "setup":
      console.log(
        "Usage: cdh setup\n\nAgents now ship with the CDH package — no setup needed.\nUpdate the @mit-sdg/cdh npm package to get the latest agents."
      );
      break;
    case "doctor":
      console.log(
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "ship": {
      ...
      const commitResult = commitShip(cwd, runId, preflight.touched);
      if (!commitResult.ok) {
    ...
    case "init": {
      ...
      console.log("  4. Start opencode in this repo to use CDH agent tools");
      break;
    }

    case "setup": {
      console.log("Agents now ship with the CDH package — no setup needed.");
      console.log("Update @mit-sdg/cdh to get the latest agents.");
      break;
    }
